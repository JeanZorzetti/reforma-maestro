...
- [ ] T005 [P] Criar `frontend-next/tests/setup.ts` que carrega `.env.local`, aborta se `DATABASE_URL_TEST` estiver ausente ou for igual a `DATABASE_URL`, aplica as migrações e expõe helper de `truncate` entre casos
...
- [ ] T014 Definir as tabelas `subscriptions`, `trial_grants`, `stripe_events` e `audit_log` em `frontend-next/src/db/schema.ts`, com `audit_log.user_id` em `ON DELETE SET NULL` e as demais FKs em `CASCADE`
...
- [ ] T022 Implementar `frontend-next/src/lib/auth.ts`: Auth.js v5 com `@auth/drizzle-adapter`, Credentials provider, `session.strategy = 'database'` e bcrypt custo 12 (R7)
...
- [ ] T026 Implementar `signUp` em `frontend-next/src/server/actions/auth.ts`: normaliza o e-mail, cria `users` com bcrypt, grava `trial_grants` e cria `subscriptions` — **em uma única transação**; se o hash já existir em `trial_grants`, a assinatura nasce `expired` (FR-025, FR-025c)
...
- [ ] T028 Escrever `frontend-next/tests/integration/auth.test.ts` cobrindo: cadastro cria as três linhas na mesma transação, e-mail duplicado é rejeitado, e-mail é normalizado antes da busca, senha nunca retorna da action, e o token de reset é de uso único
...
- [ ] T046 [P] [US1] Criar o formulário de lançamento em `frontend-next/src/components/app/lancamento-form.tsx`, com data em `dd/MM/yyyy`, seleção fechada de categoria e aviso discreto para data futura (FR-008, FR-009, FR-013)
...
- [ ] T056 [US2] Implementar os cinco handlers de evento (`checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_succeeded`, `invoice.payment_failed`), incluindo `cancel_at_period_end` → `status = 'canceled'` com acesso até `current_period_end` (FR-018, FR-021, FR-022)
...
- [ ] T058 [US2] Escrever `frontend-next/tests/integration/webhook.test.ts` cobrindo **os oito cenários obrigatórios** de [contracts/stripe-webhook.md](./contracts/stripe-webhook.md), com destaque para evento duplicado, evento fora de ordem e evento sem usuário resolvível (FR-020, R6)
...
- [ ] T063 [US2] Criar `frontend-next/src/app/api/cron/trial-warnings/route.ts` protegido por `Authorization: Bearer $CRON_SECRET`, selecionando trials em D-3 e D-1, enviando o aviso e marcando o envio para não repetir (FR-025a, R8)
...
- [ ] T075 [P] Criar a página pública de política de retenção e privacidade declarando o prazo de retenção de contas inativas e o direito de exclusão (FR-028)
...
- [ ] T077 [P] Popular uma obra de teste com 500 lançamentos e medir o carregamento do painel e da listagem; ajustar índices apenas se houver espera perceptível (SC-009)
...
- [ ] T081 Cadastrar o webhook de produção no Stripe apontando para o domínio final e configurar todas as variáveis de ambiente na Vercel
...
**Dentro da Fase 4**: T052 → T053 → T054 → T055 → T056 → T057. T058 depende de
T056. UI (T059–T064) depende de T053.
...
**Fase 7** — T075, T077, T078, T079 em paralelo.
...
oito cenários de webhook de T058 são obrigatórios antes do merge.
...
acima estiver verde — é o commit que torna a virada irreversível.
