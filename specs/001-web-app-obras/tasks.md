...
- [~] T077 [P] Popular uma obra de teste com 500 lançamentos e medir: as queries agregadas do painel devem responder em **< 200 ms (p95)** e o painel atingir **LCP < 2,5 s** em 3G rápido; ajustar índices só se algum limite for estourado (SC-009) — **parcial**: `getPainelTotais` + `getPainelPorCategoria` medidos contra 500 lançamentos no banco de teste, p95 = **42,9 ms**, bem abaixo do limite; nenhum ajuste de índice necessário. O LCP em 3G segue **não verificado** — exige Lighthouse contra um deploy real (dev server aqui aponta para o Postgres de produção via `.env.local`, então não dava para popular 500 linhas de teste nele com segurança)
- [X] T078 [P] Revisar responsividade do app em viewport de celular — o uso majoritário é celular durante a obra (Assumptions) — revisão via código (Tailwind), sem servidor dev disponível para teste ao vivo sem escrever no Postgres de produção; corrigidos 3 cabeçalhos (`obras/[id]`, `obras/[id]/lancamentos`, seletor de obras) que empilhavam título + botões sem `flex-wrap`/empilhamento e estouravam em ~375px; tabela de lançamentos já tinha `overflow-auto` (shadcn) e o gráfico já usa `ResponsiveContainer`
...
- [X] T080 ~~Confirmar `sslmode=require`~~ — descartado em 2026-08-11: tentativa de habilitar TLS no Postgres (Easypanel) colidiu com memória compartilhada de outro banco de produção ativo no mesmo host (containers não isolam IPC). Constitution v3.0.0 aceita `sslmode=disable`, mesmo padrão dos demais projetos no host; ver Sync Impact Report
...
- [X] T082 Remover o diretório `frontend/` (protótipo Lovable) do repositório (FR-035, Princípio III)
- [X] T083 Remover `frontend-next/scripts/create-spreadsheet.ts`, `populate-spreadsheet.ts`, `test-sheets.ts` e `diagnose-permissions.ts` (FR-035)
- [X] T084 Remover as dependências `googleapis` e `google-auth-library` de `frontend-next/package.json` e confirmar que o build passa (FR-035)
- [X] T085 Atualizar `docs/VISAO-GERAL-DO-PROJETO.md`, que ainda descreve o produto como landing page de planilha com checkout Kiwify
- [X] T086 Rodar o checklist final de merge do [quickstart.md](./quickstart.md): `npm test`, `npm run build`, `npm run lint`, V1-isolamento manual, e conferência de sitemap e structured data — `npm test` 55/55, `npm run build` limpo, `npm run lint` sem erro novo desta sessão (31 problemas pré-existentes em arquivos não tocados: boilerplate shadcn/ui e `banner-acesso.tsx`/`lancamento-form.tsx` em linhas alheias a esta sessão); isolamento coberto pelo automatizado (T050, 55/55) — walkthrough manual via HTTP não executado para não interferir nos dev servers já rodando; `robots.txt`, `sitemap.xml`, `noindex` em `/entrar` e structured data (`Organization` + `SoftwareApplication`, sem `aggregateRating`) conferidos contra o dev server já ativo. **`sslmode=require` de produção segue pendente — é o T080, não incluído neste checklist**
- [X] T087 [P] Instrumentar no GA4 os eventos `obra_criada`, `lancamento_criado` e `assinatura_concluida`, sem os quais SC-003 e SC-011 não têm como ser medidos após o lançamento
